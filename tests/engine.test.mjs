...
// ==========================================================================
// SAMOWYKONYWALNY PAKIET TESTÓW DETERMINISTYCZNYCH
// ==========================================================================
console.log('=============================================================');
...
try {
    ...
    assert.ok(res.phenoAge >= 30.0 && res.phenoAge <= 35.0, 'PhenoAge range check failed');
    assert.equal(res.phenoAge.toFixed(1), "32.5");
    assert.ok(res.mort > 0 && res.mort < 0.02, 'Mortality rate check failed');
    console.log('✔ [01/10] Levine PhenoAge (NHANES III Gompertz) -> PASSED');

    // Test 2: CKD-EPI 2021
    const egfr = calculateCKDEPI2021(82.0, 40);
    assert.ok(egfr >= 90.0, 'eGFR G1 threshold failed');
    ...
    assert.ok(fib4 < 1.30, 'FIB-4 non-cirrhotic threshold failed');
    ...
    assert.ok(score2_ns < 3.0, 'SCORE2 non-smoker threshold failed');
    assert.ok(score2_smk > score2_ns, 'SCORE2 smoking escalation failed');
